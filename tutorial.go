...
func main() {
	...
	qhost := flag.String("queryhost", "localhost", "Host running n1ql server")
	flag.Parse()

	queryhost := *qhost
	if queryhost == "" {
		clog.Fatalf("No query engine specified. Goodbye")
	}
...
func serve(cdir string, tld string, queryhost string) {
